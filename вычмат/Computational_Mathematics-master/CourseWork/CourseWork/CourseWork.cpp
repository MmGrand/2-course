...
double x0 = 0, xn = 1, yo = 1, z0 = -5, yk = 2.7182828;
double m1 = z0, m2 = 1.3, m3, H = 0.2, h = H/2;
double *yy, *YY, shot1, shot2, shot, eps = 0.000001;
...
double f(double x, double y, double z) {
	double result = (3*exp(x) + 4*z + 2*y) / 9;
	return result;
}
...
void shooting() {
	shot1 = RK4DP(m1);
	shot2 = RK4DP(m2);
	if (abs(shot1 - yk) < eps) printf("y подсчитан, shot1: %0.7f\n", shot1);
	...
	printf("y подсчитан, shot: %0.7f, m3 = %0.7f\n", shot, m3);
}
