...
int basic_to_asm(const char* filename_bas, const char* filename_asm) {
    ...
    while (end == 0 && fscanf(fbas, "%u %[A-Z] ", &line, buffer) != 0) {
        ...
        switch (type) {
            ...
            case 5: {
                ...
                //ќпредел€ем индексы переменных и регистрируем если требуетс€
                ...
                switch (buffer[0]) {
                    case '<':
                        //≈сли при вычитание из 2 1го число отрицательное, значит оно больше
                        ...
                        break;
                    case '>':
                        //≈сли при вычитание из 1 2e число отрицательное, значит оно больше
                        sprintf(buffer, "%02u LOAD %02u\n%02u SUB %02u\n%02u JNEG 00\n",
                        ...
                        break;
                    default:
                        //≈сли при вычитание из 2 1го число 0 - равны
                        sprintf(buffer, "%02u LOAD %02u\n%02u SUB %02u\n%02u JZ %02u\n%02u JUMP 00\n",
                                address, var[id2].address,
                                address + 1, var[id1].address,
                                address + 2, address + 4,
    ...
    //«апись в пам€ть начальных значений + констант
    //ѕроход по конфликтным переходам и заполнение их
